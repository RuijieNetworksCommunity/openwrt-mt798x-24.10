...
static BOOLEAN IAPP_DSIfInfoGet(IAPP_IN RTMP_IAPP *pCtrlBK) {
  ...
  /* get own addr */
  if (ioctl(SockIf, SIOCGIFADDR, ReqIf.ifr_name) < 0) {
    DBGPRINT(RT_DEBUG_ERROR,
  ...
  if (ioctl(SockIf, SIOCGIFBRDADDR, ReqIf.ifr_name) < 0) {
  ...
  /* get network Mask */
  if (ioctl(SockIf, SIOCGIFNETMASK, ReqIf.ifr_name) < 0) {
...
#ifdef IAPP_OS_LINUX
  if (ioctl(pCtrlBK->SocketIoctl, Param, Wrq.ifr_name) < 0) {
    DBGPRINT(RT_DEBUG_ERROR,
...
static BOOLEAN IAPP_SocketOpen(IAPP_IN RTMP_IAPP *pCtrlBK) {
  ...
  */
  ...
  if (ioctl(pCtrlBK->SocketUdpSend, SIOCGIFINDEX, ReqIf.ifr_name) != 0) {
  ...
  if (ioctl(pCtrlBK->SocketRawDrv, SIOCGIFINDEX, ReqIf.ifr_name) != 0) {
